...
class PuppeteerBenchmark {
    ...
    private buildNetworkDescription() {
        ...
        if (this.networkOptions.latency) parts.push(`${this.networkOptions.latency}ms latency`);
        if (this.networkOptions.downloadThroughput) {
            const mbps = (this.networkOptions.downloadThroughput / 1024 / 1024).toFixed(1);
            parts.push(`${mbps} Mbps down`);
        }
        if (this.networkOptions.uploadThroughput) {
            const mbps = (this.networkOptions.uploadThroughput / 1024 / 1024).toFixed(1);
    ...
    private async emulateNetwork(page: any) {
        if (!this.networkOptions.offline && !this.networkOptions.latency && 
            !this.networkOptions.downloadThroughput && !this.networkOptions.uploadThroughput) {
            return;
        }
        ...
        const conditions: any = {
            ...
            downloadThroughput: this.networkOptions.downloadThroughput || 1024 * 1024 * 10,
    ...
    private addDetailedError(number: number, type: string, message: string, details?: any) {
        this.detailedErrors.push({ number, type, message, details });
        console.log(`\n  ${colors.red}✗ Request #${number}: ${type} - ${message}${colors.nc}`);
        if (details) {
            console.log(`  ${colors.yellow}   → ${JSON.stringify(details)}${colors.nc}`);
        }
    }
    ...
    async doRequest(number: number, url: string): Promise<RequestTiming> {
        ...
        try {
            ...
            await page.setUserAgent('Mozilla/5.0 (X11; Ubuntu; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/148.0.0.0 Safari/537.36');
            ...
            if (navigationError) {
                ...
                if (errorMsg.includes('ERR_CONNECTION_REFUSED')) type = 'CONNECTION_REFUSED';
                else if (errorMsg.includes('ERR_NAME_NOT_RESOLVED')) type = 'DNS_ERROR';
                else if (errorMsg.includes('ERR_SSL_PROTOCOL_ERROR')) type = 'SSL_ERROR';
                else if (errorMsg.includes('ERR_QUIC_PROTOCOL_ERROR')) type = 'QUIC_ERROR';
            ...
            this.addDetailedError(
                ...
                { totalTime, responseReceived, responseStatus }
            );
            return {
                number,
                status: 0,
            ...
            this.addDetailedError(number, 'EXCEPTION', error.message || 'Unknown exception');
            return {
                number,
                status: 0,
                timeTotal: 0,
    ...
    async run(requests: number, url: string, useHttp3: boolean = false, forceQuicOn?: string): Promise<BenchmarkResults> {
        ...
        try {
            ...
            this.addDetailedError(0, 'INIT_ERROR', `Failed to initialize browser: ${error.message}`);
            throw error;
        }
        ...
        for (let i = 1; i <= requests; i++) {
            ...
            process.stdout.write(`\r    [${bar}] ${progress}% - Request #${i}${' '.repeat(10)}`);
        }
    ...
    private analyzeResults(results: RequestTiming[], protocol: string): BenchmarkResults {
        const successful = results.filter(r => r.status >= 200 && r.status < 400);
        const failed = results.filter(r => r.status === 0 || r.status >= 400 || r.error);
...
function printResults(results: BenchmarkResults) {
    ...
    console.log(`  ${colors.green}✓ Successful:${colors.nc} ${results.successful}`);
    console.log(`  ${colors.red}✗ Errors:${colors.nc} ${results.failed - results.timeoutCount}`);
    console.log(`  ${colors.yellow}⏱ Timeouts:${colors.nc} ${results.timeoutCount}`);
    ...
    if (results.statusCodes.size > 0) {
        ...
        for (const [code, count] of results.statusCodes) {
            ...
            console.log(`    ${colorCode}${code}${colors.nc}: ${count} (${percent.toFixed(1)}%)`);
        }
        console.log();
    }

    if (results.detailedErrors && results.detailedErrors.length > 0) {
        console.log(`  ${colors.red}✗ Detailed Errors (${results.detailedErrors.length}):${colors.nc}`);
        ...
        const sortedErrors = Array.from(errorsByNumber.entries()).sort((a, b) => a[0] - b[0]);
    ...
    if (results.failedNumbers.length > 0) {
        ...
        const suffix = results.failedNumbers.length > 20 ? ` ... (${results.failedNumbers.length} total)` : '';
        console.log(`  ${colors.red}Failed requests list:${colors.nc} ${nums}${suffix}`);
        console.log();
    }
}

function printComparison(results2: BenchmarkResults, results3: BenchmarkResults) {
    console.log(`\n${colors.blue}${'═'.repeat(70)}${colors.nc}`);
    console.log(`${colors.blue}${colors.bold}  📊 COMPARISON: HTTP/2 vs HTTP/3${colors.nc}`);
    console.log(`${colors.blue}${'═'.repeat(70)}${colors.nc}`);
    console.log('');
    
    console.log(`  ${colors.cyan}HTTP/2:${colors.nc}  ${results2.avgTime.toFixed(4)}s (${results2.successful} successful)`);
    console.log(`  ${colors.magenta}HTTP/3:${colors.nc}  ${results3.avgTime.toFixed(4)}s (${results3.successful} successful)`);
    console.log('');
    
    console.log(`  ${colors.yellow}Percentiles comparison:${colors.nc}`);
    console.log(`    P50:  HTTP/2 ${results2.p50.toFixed(4)}s  vs  HTTP/3 ${results3.p50.toFixed(4)}s`);
    console.log(`    P90:  HTTP/2 ${results2.p90.toFixed(4)}s  vs  HTTP/3 ${results3.p90.toFixed(4)}s`);
    console.log(`    P99:  HTTP/2 ${results2.p99.toFixed(4)}s  vs  HTTP/3 ${results3.p99.toFixed(4)}s`);
    console.log('');
    
    if (results2.successful > 0 && results3.successful > 0 && 
        results2.avgTime > 0 && results3.avgTime > 0) {
        
        if (results2.avgTime < results3.avgTime) {
            const diff = ((results3.avgTime - results2.avgTime) / results2.avgTime * 100);
            const absDiff = results3.avgTime - results2.avgTime;
            console.log(`  ${colors.green}🏆  HTTP/2 is faster by ${diff.toFixed(2)}% (${absDiff.toFixed(4)}s)${colors.nc}`);
        } else if (results3.avgTime < results2.avgTime) {
            const diff = ((results2.avgTime - results3.avgTime) / results3.avgTime * 100);
            const absDiff = results2.avgTime - results3.avgTime;
            console.log(`  ${colors.green}🏆  HTTP/3 is faster by ${diff.toFixed(2)}% (${absDiff.toFixed(4)}s)${colors.nc}`);
        } else {
            console.log(`  ${colors.yellow}⚖️  Protocols are equal${colors.nc}`);
        }
    } else {
        console.log(`  ${colors.red}❌  Not enough data for comparison${colors.nc}`);
    }
    console.log('');
...
const downloadThroughput = downloadMbps > 0 ? downloadMbps * 1024 * 1024 : undefined;
...
async function main() {
    ...
    console.log(`  Network: ${networkParts.length > 0 ? networkParts.join(', ') : 'Normal (no emulation)'}`);
    ...
    const report = {
        ...
        comparison: {
            winner: results2.avgTime < results3.avgTime ? 'HTTP/2' : 
                    results3.avgTime < results2.avgTime ? 'HTTP/3' : 'equal',
            http2_avg: results2.avgTime,
            http3_avg: results3.avgTime,
            speed_diff_percent: results2.avgTime < results3.avgTime ? 
